...
function setToArray(mySet) {
  ...
  // Використовуємо метод filter для створення нового масиву, що містить лише рядкові елементи.
	let filteredArray = newArray.filter((item) => typeof(item) === "string");
...
// Виведе: false
console.log(areDisjoint(new Set([1, 2]), new Set([6, 7, 8])));
console.log(areDisjoint(new Set([6, 5, 4]), new Set([6, 7, 8])));
...
function getDifference(set1, set2) {
  ...
  // Перебір елементів першої множини за допомогою оператору for of
	for (const item of set1) {
		if (!set2.has(item)) differenceSet.add(item);
	}
